...
<HTML>
   <HEAD>
      ...
      <TITLE>Upload POS For Group</TITLE>
      <center>
      <h2>Upload POS For Group</h2>
      </center>
   </HEAD>
...
function submitForm()
{
  ...
  document.myform.action = document.myform.action+'&'+'<%out.print(DCMInterfaceKey.INPUT_HIDDEN_GROUP_ID+"=");%>'+document.myform.<%out.print(DCMInterfaceKey.INPUT_HIDDEN_GROUP_ID);%>.value
...
String appName = request.getContextPath();
    ...
    Vector groupListVector = (Vector)dataHashMap.get(InterfaceKey.HASHMAP_KEY_COLLECTION);
    
    String formAction = appName +"/servlet/com.mobinil.sds.web.controller.WebControllerServlet?"
                    +InterfaceKey.HASHMAP_KEY_ACTION+"="
                    +DCMInterfaceKey.ACTION_DCM_UPLOAD_POS_FOR_GROUP_PROCESS+
...
<%                  
   ...
   // out.println("<td nowrap align=left>");
    ...
    out.println("<td nowrap align=left>");
%>
   <select name="<%=DCMInterfaceKey.INPUT_HIDDEN_GROUP_ID%>" id="<%=DCMInterfaceKey.INPUT_HIDDEN_GROUP_ID%>">
    <%
    for(int g=0;g<groupListVector.size();g++)
    {
      GroupModel groupModel = (GroupModel)groupListVector.get(g);
