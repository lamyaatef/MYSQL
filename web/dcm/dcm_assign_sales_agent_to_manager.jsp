...
<SCRIPT language="javascript">
  var arrUsersAtNextLevelForTheSameManager=new Array
             ( 
  <%fillInUsersAtNextLevel(request,response,out);%>                    
             )
...
<%!
  ...
  { 
    ...
    out.println("<CENTER>");
    out.println("  <H2>Assign Users To Users</H2>");
    out.println("</CENTER>");
    ...
    out.println("<input type=\"hidden\" name=\""+InterfaceKey.HASHMAP_KEY_ACTION+"\""+
                  " value=\""+DCMInterfaceKey.ACTION_DCM_SAVE_ASSIGN_SALES_AGENT_TO_MANAGER+"\">");
    ...
    out.println("<TR>");
    out.println("  <TD class=TableHeader vAlign=top colSpan=2>Sales Agents");
    ...
    out.println("   user_defined_data_view.ColumnHeaders.add(null,null,\"Sales Agent\",null,200,\"center\",DG_SELECT,arrUsersAtNextLevelForTheSameManager,null,null)   ");
    ...
    Vector usersAssingedToSameManagerButNextLevel = (Vector)dataHashMap.get(InterfaceKey.HASHMAP_KEY_COLLECTION);       
    Vector usersAssingedToSameManagerButNextLevelIds = new Vector();
    for (int i = 0; i <usersAssingedToSameManagerButNextLevel.size() ; i++) 
    {
      DCMUserModel dcmUserModel = (DCMUserModel)usersAssingedToSameManagerButNextLevel.get(i);
      String XdcmUserId = dcmUserModel.getDcmUserId();
      String XuserId = dcmUserModel.getUserId();
      usersAssingedToSameManagerButNextLevelIds.add(XuserId);
    }
    Vector userAssignedTo = (Vector)dataHashMap.get(InterfaceKey.HASHMAP_KEY_ADDITIONAL_COLLECTION_2);
    int positionFromLast = userAssignedTo.size();
    int positionFromFirst = 1;
    for(int k=1;k<=userAssignedTo.size();k++)
    {
      String strUserAssignedToId = (String)userAssignedTo.get(k-1); 
      if(usersAssingedToSameManagerButNextLevelIds.contains(strUserAssignedToId))
      {
        out.println("<script>");
        out.println("user_defined_data_view.RowSet.add("+positionFromFirst+");");
        out.println("eval(\"document.formDataView.user_defined_data_view__R"+positionFromFirst+"__C1.value = '"+ strUserAssignedToId +"';\");");
        out.println("</script>");

        out.println("<input type=\"hidden\" name=\"user_defined_data_view__R"+positionFromFirst+"__C3\""+
                  " value=\""+strUserAssignedToId+"\">");
                  
        positionFromFirst++;
      }
      else
      {
        out.println("<input type=\"hidden\" name=\"user_defined_data_view__R"+positionFromLast+"__C1\""+
                  " value=\""+strUserAssignedToId+"\">"); 

        out.println("<input type=\"hidden\" name=\"user_defined_data_view__R"+positionFromLast+"__C3\""+
                  " value=\""+strUserAssignedToId+"\">");           
...
<%!
  public void fillInUsersAtNextLevel(HttpServletRequest request, HttpServletResponse response, JspWriter out)
  throws ServletException, IOException
  {
      HashMap hmDataHashMap = new HashMap(100);
      hmDataHashMap = (HashMap)request.getAttribute(InterfaceKey.HASHMAP_KEY_DTO_OBJECT);
      Vector usersAssingedToSameManagerButNextLevel = (Vector)hmDataHashMap.get(InterfaceKey.HASHMAP_KEY_COLLECTION);       
      if(usersAssingedToSameManagerButNextLevel != null)
      for (int i = 0; i <usersAssingedToSameManagerButNextLevel.size() ; i++) 
      {
        DCMUserModel dcmUserModel = (DCMUserModel)usersAssingedToSameManagerButNextLevel.get(i);
        ...
        out.println("new Array('"+userFullName+"',"+userId+")");
        if(i<usersAssingedToSameManagerButNextLevel.size()-1)
        out.println(",");
      }
  }
