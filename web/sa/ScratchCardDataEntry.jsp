...
{
 var fieldsArray = document.scratchCardForm.<%out.print(AdministrationInterfaceKey.CONTROL_INPUT_AMOUNT);%>;
...
<%
   ...
   Hashtable scratchCardTable = (Hashtable) dataHashMap.get(InterfaceKey.HASHMAP_KEY_COLLECTION);
   ...
   out.println("<form action=\""+appName+"/servlet/com.mobinil.sds.web.controller.WebControllerServlet\" name=\"scratchCardForm\" method=\"post\">"); 
   ...
   out.println("<input type=\"hidden\" name=\""+InterfaceKey.HASHMAP_KEY_USER_ID+"\" "+
    ...
    {
      out.println("<script>document.scratchCardForm."+AdministrationInterfaceKey.CONTROL_SELECT_MONTH+".options["+month+"].selected=true;" + "</script>");
    }
    out.println("</TD>");
    out.println("</TR>");    
    out.println("<TR>");
    ...
    out.print("onclick=\"if (checkBeforeView(document.scratchCardForm."+AdministrationInterfaceKey.CONTROL_INPUT_YEAR+")){ document.scratchCardForm."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+
    AdministrationInterfaceKey.ACTION_SEARCH_SCRATCH_CARD+"'; document.scratchCardForm.submit(); } else alert('Please Enter A Valid Year');\">");
    ...
    if (dcmDto!=null)
    {
      ...
      for (int i = 0 ; i <dcmDto.getDcmModelsSize();i++)
      {
        ...
        ScratchCardModel scratchCardModel = (ScratchCardModel) scratchCardTable.get(model.getDcmId()+"");
        String value ="";
        String revenueId = "";
        ...
        {
          value=scratchCardModel.getAmount() +"";
         revenueId =  scratchCardModel.getRowID();
        }
        
        out.println("<td nowrap align=center>");
        
        out.println("<input type=\"HIDDEN\" name=\""+AdministrationInterfaceKey.CONTROL_INPUT_SCRATCH_CARD_ID+"\" value=\""+revenueId+"\">");        out.println("<input type=\"HIDDEN\" name=\""+AdministrationInterfaceKey.CONTROL_INPUT_DCM_ID+"\" value=\""+model.getDcmId()+"\">");
      ...
      out.print("onclick=\"if (checkBeforeUpdate()) {document.scratchCardForm."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+
      AdministrationInterfaceKey.ACTION_UPDATE_SCRATCH_CARD+"'; document.scratchCardForm.submit();} else {alert('one or more data entry value is Invalid');} \">");
