        ...
        if(newBatchDto != null)
        {
          ...
          if(contractDtos != null)
          {
            ...
            for(int j=0; j<contractDtos.size(); j++)
            {
              ...
              //SheetModel sheetModel  = (SheetModel) sheetsStatistic.get(sheetName);
              ...
              boolean isSheetReImported = sheetsReimportedTable.containsKey(sheetID);
              ...
              out.println("<TABLE style=\"BORDER-COLLAPSE: collapse\" cellSpacing=2 cellPadding=1 width=\"100%\" border=1 id=\"table_"+sheetID+"\" name=\"table_"+sheetName +"\">");
              out.println("<TR class=TableHeader>");
              out.println("<td align=middle>Sheet Serial Number</td>");
              out.println("<td align=middle>Contract Main Sim No</td>");
              out.println("<td align=left>Contract Type</td>");
              out.println("<td align=middle>Contract Status<br>");
              out.println("<Select name="+ContractRegistrationInterfaceKey.CONTROL_SELSCT_NAME_PREFIX+sheetID+" onchange=\"changeAll(this);\">");
              out.println("<option>Apply status to all sheet Contracts</option>");
              ...
              out.println("<td align=middle>Contract Warning<br>");
              
              out.println("<Select name=\""+ContractRegistrationInterfaceKey.CONTROL_SELECT_USER_WARNINGS+"_"+sheetID+"\" onchange=\"changeAll(this);\">");
              ...
              {                                          
                ...
                Vector contractWarningVector = newContractDto.getContractWarning(contractID);
                //Utility.logger.debug("contractWarningVector size = " + contractWarningVector); 
                
                boolean reimported = newContractModel.getReImportedFlag();

                if(reimported || isSheetReImported)
                {
                ...
                out.println("<td nowrap align=center>"+sheetSerialNum+"</td>");
                if(reimported || isSheetReImported)
                {
                ...
                else
                {
                  ...
                  "document.AuthCall."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+AuthCallInterfaceKey.ACTION_ADMIN_SHOW_CONTRACT_DETAILS_FOR_AUTHENTICATION_CALL+"'; "+
                ...
                out.println("<td nowrap align=left align=center>"+contractStatus+"<br>");

              if(!reimported  && !isSheetReImported)
                {
                  out.println("<Select name="+ContractRegistrationInterfaceKey.CONTROL_SELSCT_NAME_PREFIX+sheetID+"_"+contractID+">");
                  out.println("<option></option>");
                  for(int l= 0; l<newContractStatusTypeDto.getContractStatusTypeModelsSize(); l++)
                  {
                    int contractStatusTypeID = newContractStatusTypeDto.getContractStatusTypeModel(l).getId();
                    String contractStatusTypeName = newContractStatusTypeDto.getContractStatusTypeModel(l).getName();
/*
                    if (newContractModel.getStatusTypeName().equals(contractStatusTypeName))
                    {
                    contractStatusId = contractStatusTypeID;
                ...
                out.println("<input type=\"hidden\" name=\""+AuthCallInterfaceKey.CONTROL_HIDDEN_SHEET_CONTRACT_OLD_STATUS+sheetID+"_"+contractID+"\" value=\""+contractStatusId+"\">");
                ...
                if(!reimported && !isSheetReImported)
                {
                  out.println("<Select name=\""+ContractRegistrationInterfaceKey.CONTROL_SELECT_USER_WARNINGS+"_"+sheetID+"_"+contractID+"\" onchange=\"changeAll(this);\">");
                  out.println("<option></option>");
                  for(int l=0; l<userWarning.size(); l++)
                  {
                    ContractWarningModel userWarningModel = (ContractWarningModel)userWarning.elementAt(l);
          ...
          out.println("onclick=\"document.AuthCall."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+
                      AuthCallInterfaceKey.ACTION_ADMIN_SEARCH_BATCH_AUTHENTICATION_CALL+"';AuthCall.submit();\">");
          out.print("</td>");
          out.print("</tr>");
