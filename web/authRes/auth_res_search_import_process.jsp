...
<%
   ...
   //   String operation =(String)request.getParameter(AdministrationInterfaceKey.QUERY_STRING_OPERATION);
    //  String strUserID =(String)request.getParameter(InterfaceKey.HASHMAP_KEY_USER_ID);
      ...
      String catId=(String)request.getParameter(AuthResInterfaceKey.CONTROL_SELECT_SEARCH_CATEGORY_ID);
      String fileTypeId=(String)request.getParameter(AuthResInterfaceKey.CONTROL_SELECT_FILE_TYPE_ID);
      Boolean simPure=Boolean.valueOf( (String)request.getParameter(AuthResInterfaceKey.CONTROL_COMBO_SEARCH_BY_ID));
      
      String desc=(String)request.getParameter(AuthResInterfaceKey.CONTROL_INPUT_DESCRIPTION);
    ...
    %>
          ...
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=AuthResInterfaceKey.ACTION_AUTH_RES_SEARCH_IMPORT %>">
  ...
  %>
    ...
    <script>
  	alert('FILE NOT EXIST OR DELETED  YEAR ' + '<%=year%>'+'  ,month '+ '<%=month%> and Label  '+ '<%=labelName%> ');
    ...
    <%
      ...
      uploadAuthResThread upResTH= new  uploadAuthResThread(year,month,"Processing",label,desc,strUserID,baseDirectory+fileUniqueName,catId,fileTypeId,simPure);
      
      
      upResTH.start();
      
     out.print("<script>");
     out.print("document.import_res.action.value = '"+AuthResInterfaceKey.ACTION_AUTH_RES_SEARCH_IMPORT+"';" );
