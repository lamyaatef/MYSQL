<%-- 
    Document   : upload_voc_mnp_migra
    Created on : Jan 24, 2012, 12:58:03 PM
    Author     : mabdelaal
--%>
...
<html>
    <head>
        ...
        <title>Voice MNP Achievement File Upload</title>
    </head>
    <%
    ...
    <body>
        <form name="uploadform" id="uploadform" action="" method="POST" enctype="multipart/form-data">
            <table style="BORDER-COLLAPSE: collapse" width="60%" border="1" align="center" cellpadding="0" cellspacing="1">
                ...
                <tr>
                <td class=TableHeader width="20%">Label</td>
                <td class=TableHeader width="80%">
                        <select  name="<%=AuthResInterfaceKey.CONTROL_SELECT_LABEL_ID %>" id="<%=AuthResInterfaceKey.CONTROL_SELECT_LABEL_ID %>">
                            <option selected="selected" value="0" label="---Select---">---Select---</option>                            
                            <option value="1" label="Voice">Voice</option>                            
                            <option value="4" label="MNP">MNP</option>
                            <option value="5" label="Migration">Migration</option>
                        </select>
                    </td>
                    </tr>
                ...
                <td class=TableHeader width="80%">                        
                    ...
                    <input type="hidden" name="<%=AuthResInterfaceKey.CONTROL_HIDDEN_UPLOAD_FILE_TYPE_ID %>" id="<%=AuthResInterfaceKey.CONTROL_HIDDEN_UPLOAD_FILE_TYPE_ID %>" value="1"/>
        </form>
        <script>
            <%if (errorMsg!=null) out.println("alert('"+errorMsg+"')");%>
                        function checkBeforeSubmit(){                            
                            var typeId = document.uploadform.<%=AuthResInterfaceKey.CONTROL_SELECT_LABEL_ID %>.value;
                            ...
                            var label = document.uploadform.<%=AuthResInterfaceKey.CONTROL_SELECT_LABEL_ID %>.value;
                            ...
                            if (msg!="")
                                ...
                                {
                                 actionStr += '<%=InterfaceKey.HASHMAP_KEY_ACTION%>=<%=AuthResInterfaceKey.ACTION_UPLOAD_VOC_MNP_MIGRA_PROCESS %>';
                                actionStr +='&';                            
                                ...
                                actionStr +='&';
                                actionStr += '<%=AuthResInterfaceKey.CONTROL_SELECT_LABEL_ID%>='+label;
