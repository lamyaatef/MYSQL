...
<body>
<center><H2><%=((FactorModel)((Vector)paymentCommissionFactors.get(0)).get(0)).getCommissionName()%></H2></center>
...
for(int i = 0 ; i < paymentCommissionFactors.size() ; i++)
{
  ...
  else
  {
    ...
    else if(j+1 < factors.size() && !oldDCM.equals(((FactorModel)factors.get(j+1)).getCommissionDCMCode()))
    {
      ...
      subTotalNotEligibleItems +=  factorModel.getCommissionItemAmount();    
      out.println("<tr><td class=TableHeader colspan=2> ["+oldDCM+"] Total Non Eligible Items</td><td class=TableHeader colspan=1>"+subTotalNotEligibleItems+
                  "</td><td class=TableHeader colspan=2> ["+oldDCM+"] Total Eligible Items</td><td class=TableHeader colspan=1>"+subTotalEligibleItems+"</td>"+
                  "<td class=TableHeader colspan=2> ["+oldDCM+"] Total Payment</td><td class=TableHeader colspan=1>"+subTotal+"</td></tr>");
...
out.println("<td colspan=1  class=TableHeader>"+totalNotEligibleItems+"</td>");
out.println("<td class =TableHeader colspan=2>Total Eligible</td>");
out.println("<td colspan=1  class=TableHeader>"+totalEligibleItems+"</td>");
out.println("<td class =TableHeader colspan=2>Total Payment</td>");
out.println("<td colspan=1  class=TableHeader>"+totalPayment+"</td>");
out.println("</tr>");
