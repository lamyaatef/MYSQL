...
String appName = request.getContextPath();
  ...
  String appendFile = (String) dataHashMap.get(DMInterfaceKey.HIDDEN_ACTION_2);
  
  String FileId = (String) dataHashMap.get(DMInterfaceKey.HIDDEN_FILE_ID);
  
  String year=(String)dataHashMap.get(DMInterfaceKey.CONTROL_SELECTED_YEAR);
 //String year="2009";

System.out.println("year in jsp "+year);
  String month=(String)dataHashMap.get(DMInterfaceKey.CONTROL_SELECTED_MONTH);
// String month="3";

System.out.println("month in jsp "+month);    //stem.out.println("user id ssssssssss"+strUserID);
   
    
    String formAction = appName +"/servlet/com.mobinil.sds.web.controller.WebControllerServlet?"+InterfaceKey.HASHMAP_KEY_ACTION+"="+DMInterfaceKey.ACTION_AUTH_CALL_IMPORT_PROCESS+"&"+InterfaceKey.HASHMAP_KEY_USER_ID+"="+strUserID;
    formAction.trim();
...
function submitForm()
{
  ...
  <%if( appendFile != null) {%> 
  document.myform.action = document.myform.action+'&'+'<%out.print(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_YEAR);%>='+<%out.print(year);%>;
  document.myform.action = document.myform.action+'&'+'<%out.print(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_MONTH);%>='+<%out.print(month);%>;


  document.myform.action = document.myform.action+'&'+'appendFile=notequalnull';
  document.myform.action = document.myform.action+'&'+'<%out.print(DMInterfaceKey.HIDDEN_FILE_ID);%>='+<%out.print(FileId);%>;

  

  <%}
  else{%>
  var year1= document.myform.<%out.print(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_YEAR);%>.value;
  var month1 = document.myform.<%out.print(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_MONTH);%>.value;
  document.myform.action = document.myform.action+'&'+'<%out.print(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_YEAR);%>='+year1;
  document.myform.action = document.myform.action+'&'+'<%out.print(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_MONTH);%>='+month1;
  document.myform.action = document.myform.action+'&'+'appendFile=equalnull'; 
  <%}%> 
...
<HTML>
   <HEAD>
      ...
      <TITLE>Authintcation Call Import</TITLE>
      
      
      <center>
      <h2>Authintcation Call Import</h2>
      </center>
   </HEAD>
...
<form name="myform" action="<%out.print(formAction);%>"method="post" enctype="multipart/form-data">
            ...
            <%if( appendFile == null) {%>    
                ...
                <td width="15%">Year:</td>
                <td>
                <select name="<%=DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_YEAR %>" id="<%=DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_YEAR %>">
                <option></option>
                ...
                <td>Month:</td>
                <td>
                <select  name="<%=DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_MONTH%>" id="<%=DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_MONTH%>">
                <option></option>
                <%
