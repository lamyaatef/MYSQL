...
<%
      ...
      String year=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_POS_ELG_YEAR);
      String month=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_POS_ELG_MONTH);
      
      if( DataMigrationDao.checkPOS_ELGFile(year,month))
      {
    	  
  %>

  <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="Import" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_POS_ELG_CHK_IMPORT %>">
  </form>
    <script>
   ...
   //   boolean checkFromLCSPhysicalAmount = false;
    //  if(vecCurrentStockProducts.size()==0)
     // {
        Vector vec= DataMigrationDao.getTmpPOS_ELG_CHKData();
      ...
      if(vec.size()!=0){
    	 ...
    	 fileid= DataMigrationDao.insertNewPOS_ELG_CHKFile(year,month,"active",userID);  
    	  
      }  
        
        for(int i = 0;i<vec.size();i++)
        {
          POS_ELG_CHKModel model = (POS_ELG_CHKModel) vec.get(i);
          
          
          
          String posCode =model.getPOS_CODE();
          
          String posEnm =model.getPOS_ENM();
         
          String entryDt =model.getENTRY_DT();
          ...
          String stkDlvrDate =model.getSTK_DLVR_DT();
          
          String iqrarDlvrDate =model.getIQRAR_DLVR_DT();
          
          String iqrarRcvDate =model.getIQRAR_RCV_DT();
          
          String posStatus =model.getPOS_STATUS();
          
          String stkStatus =model.getSTK_STATUS();
          
          String regName =model.getREGIONAL_NAME();
          
          
          String repName =model.getREP_NAME();
          
          String channelCode =model.getCHANNEL_CODE();
          String levelCode =model.getLEVEL_CODE();
          
       String    stkdail=model.getSTK_DIAL_NO();
    ...
    DataMigrationDao.insertNewPOS_ELG_CHKData(fileid,posCode,posEnm,entryDt,stkDlvrDate,iqrarDlvrDate,iqrarRcvDate,posStatus, stkStatus ,regName,repName,channelCode,levelCode,stkdail) ;     
