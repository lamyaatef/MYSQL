...
<%
      ...
      String year=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_YEAR);
      String month=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_AUTH_CALL_MONTH);
      
      String appendfFile=(String)request.getParameter("appendFile");
      
      System.out.println("the   append file in  auth_call_process"+appendfFile);
      
      if( DataMigrationDao.checkAuthCall(year,month)  && appendfFile.equalsIgnoreCase("equalnull"))
      {
    	  
  %>

  <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="Import" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>"  value="<%=DMInterfaceKey.ACTION_AUTH_CALL_IMPORT %>">
  </form>
    <script>
   ...
   //   boolean checkFromLCSPhysicalAmount = false;
    //  if(vecCurrentStockProducts.size()==0)
     // {
        Vector vec= DataMigrationDao.getTmpAuthCall();
        //if dcm delete else don't
       // if(strChannelId.equals("1"))
        //SchemaDAO.deleteProductEquationCalculate();
        Long fileid=null;
        
      if(vec.size()!=0  &&  appendfFile.equalsIgnoreCase("equalnull"))
        {  
    	 fileid= DataMigrationDao.insertNewAuthCallFile(year,month,"active",userID);  
    	 } 
      ...
      else
      {
         ...
         String ddd= (String) request.getParameter(DMInterfaceKey.HIDDEN_FILE_ID);
         fileid= new Long(Long.parseLong(ddd));
        ...
        {
          authCallModel model  = (authCallModel) vec.get(i);
          String tmRowNum = model.getROW_NUM();
          String tmdcmCode  =model.getDCM_CODE();
          String realcount =model.getUNREAL_COUNT();
          
   
  //  Long fileid= DataMigrationDao.insertNewPaymentCashFile(year,month,"active");
    
    DataMigrationDao.insertNewAuthCallData(fileid,tmdcmCode,realcount);
    ...
    //else
     ...
     //   contractError.setErrorMsg("Stock Products have values for today");
      ...
      // }
     DataMigrationDao.deleteTmpAuthCallData();
