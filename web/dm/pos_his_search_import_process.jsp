...
<%@page import="java.sql.Statement"%><form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="import_res" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_HIS_SEARCH_IMPORT %>">
   ...
   //  desc,strUserID,baseDirectory+fileUniqueName
      ...
      try {
			...
			try {
		      ...
		      try {
               ...
               while ((line = input.readLine()) != null) {
					    	  ...
					    	  if(count==1 ){
									 fileid=DataMigrationDao.insertPayHisSearchFile(year,month,"processing",desc ,strUserID);  	 
					                     }
									//	System.out.println("line issss "+line);
										...
										//	if(DataMigrationDao.checkIfposCodeExistHis(v1,year,month))
									//		{
											
											batchcount++;
										    DataMigrationDao.insertPOSHisSearchData(fileid,v1,year,month,stat);
											if(batchcount==500)
											{	
               ...
               stat.executeBatch();
               stat.close();
               DataMigrationDao.updatePOSPayHisExistFlag(fileid,month,year);
			...
			//	counter.incCounter(countErrors + "");
			 ...
			 DataMigrationDao.updateSearchpos_HIS_file(con,fileid.toString(), "active");
     ...
     out.print("<script>");
     out.print("document.import_res.action.value = '"+DMInterfaceKey.ACTION_HIS_SEARCH_IMPORT+"';" );
