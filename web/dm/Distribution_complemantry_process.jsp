...
<%
      ...
      String year=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_COMP_DIST_YEAR);
      String month=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_COMP_DIST_MONTH);
      
      if( DataMigrationDao.checkDistFile(year,month))
      {
    	  
  %>

  <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="Import" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_COMPLEMANTRY_DISTRIBUTION_IMPORT %>">
  </form>
    <script>
   ...
   //   boolean checkFromLCSPhysicalAmount = false;
    //  if(vecCurrentStockProducts.size()==0)
     // {
        Vector vec= DataMigrationDao.getTmpDistComp();
      ...
      if(vec.size()!=0){
    	 ...
    	 fileid= DataMigrationDao.insertNewCompDistFile(year,month,"active",userID);  
    	  
      }  
        
        for(int i = 0;i<vec.size();i++)
        {
          CompDistModel model = (CompDistModel) vec.get(i);
          String tmRowNum =model.getROW_NUM();
          String tmScmCode =model.getSCM_CODE();
          String etopup =model.getETOPUP();
          String SCS =model.getSCS();
  ...
  //  Long fileid= DataMigrationDao.insertNewPaymentCashFile(year,month,"active");
    ...
    DataMigrationDao.insertNewCompDistData(fileid,tmScmCode,SCS,etopup);
    ...
    //else
     ...
     //   contractError.setErrorMsg("Stock Products have values for today");
      ...
      // }
     DataMigrationDao.deleteTmpCompDistData();
