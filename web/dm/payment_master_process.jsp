...
<%
      ...
      String year=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_PAYMENT_MASTER_YEAR);
      String month=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_PAYMENT_MASTER_MONTH);
      if( DataMigrationDao.checkMasterFile(year,month))
      {
    	  
  %>

  <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="Import" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_PAYMENT_MASTER_IMPORT %>">
  </form>
    <script>
   ...
   //   boolean checkFromLCSPhysicalAmount = false;
    //  if(vecCurrentStockProducts.size()==0)
     // {
        Vector vec= DataMigrationDao.getTmpPaymentMaster();
      ...
      if(vec.size()!=0){
    	 ...
    	 fileid= DataMigrationDao.insertNewPaymentMasterFile(year,month,"active",strUserID);  
    	  
      }  
        
        for(int i = 0;i<vec.size();i++)
        {
          PaymentMasterModel model = (PaymentMasterModel) vec.get(i);
          String tmRowNum =model.getROW_NUM();
          String tmCustCode =model.getCUSTOMER_CODE();
          String mbNo =model.getMOBILE_NUMBER();
          String tmName =model.getNAME();
          String tmPayDate =model.getPAYMENT_DATE();
          String tmPaySerial =model.getPAYMENT_SERIAL();
          String tmPayCredit =model.getCREDIT_CARD_ACCOUNT_NO();
          String extraPay =model.getEXTRA_PAYMENT();
          String tmTotAmount=model.getTOTAL_AMOUNT();
  
    DataMigrationDao.insertNewPaymentMasterData(fileid,tmName,tmPaySerial,extraPay,tmPayDate ,tmCustCode ,mbNo,tmPayCredit,tmTotAmount);

       }
    
     DataMigrationDao.deleteTmpPaymentMasterData();
