...
<%
      ...
      String year=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_PAYMENT_VISA_YEAR);
      String month=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_PAYMENT_VISA_MONTH);
      
     
   
    
    if( DataMigrationDao.checkVisaFile(year,month))
    {
  	  
%>

<form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="Import" method="post">
        <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_PAYMENT_VISA_IMPORT%>">
</form>
  <script>
  ...
  <%
   ...
   //   boolean checkFromLCSPhysicalAmount = false;
    //  if(vecCurrentStockProducts.size()==0)
     // {
        Vector vec= DataMigrationDao.getTmpvisaData();
      ...
      if(vec.size()!=0){
    	 ...
    	 fileid= DataMigrationDao.insertNewPaymentVisaFile(year,month,"active",strUserID);  
    	  
      }  
        
        for(int i = 0;i<vec.size();i++)
        {
          PaymentVisaModel model = (PaymentVisaModel) vec.get(i);
          String tmRowNum =model.getROW_NUM();
          String tmCustCode =model.getCUSTOMER_CODE();
          String mbNo =model.getMOBILE_NUMBER();
          String tmName =model.getSCM_CODE();
          String tmPayDate =model.getPAYMENT_DATE();
          String tmPaySerial =model.getPAYMENT_SERIAL();
       
          String tmTotAmount=model.getTOTAL_AMOUNT();
  
    DataMigrationDao.insertNewPaymentVisaData(fileid,tmName,tmPaySerial,tmPayDate ,tmCustCode ,mbNo,tmTotAmount);
    
    

       }
    
     DataMigrationDao.deleteTmpPaymentVisaData();
