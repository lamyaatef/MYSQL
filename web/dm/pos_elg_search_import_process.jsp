...
<form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="import_res" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_ELG_CHK_SEARCH_IMPORT %>">
  ...
  %>
   ...
   Statement   stat = con.createStatement();
			...
			try {
		      ...
		      try {
               ...
               while ((line = input.readLine()) != null) {
					    	  ...
					    	  if(count==1 ){
									 fileid=DataMigrationDao.insertPosElgSearchFile(year,month,"processing",desc ,strUserID);  	 
					                     }
										...
										//	if(DataMigrationDao.checkIfposCodeExist(v1,year,month))
										//	{
												DataMigrationDao.insertPOSElgSearchData(fileid,v1,year,month,stat);
									//		}
										//	else{ba
											batchcount++;
												if(batchcount==500)
										...
										//		DataMigrationDao.insertInvalidPOScODE(fileid.toString(),v1);
               ...
               DataMigrationDao.updatePOSElgExistFlag(fileid,month,year);
			...
			//	counter.incCounter(countErrors + "");
			 ...
			 DataMigrationDao.updateSearchpos_elg_file(con,fileid.toString(), "active");
     ...
     out.print("<script>");
     out.print("document.import_res.action.value = '"+DMInterfaceKey.ACTION_ELG_CHK_SEARCH_IMPORT+"';" );
