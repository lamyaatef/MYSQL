...
<%
      ...
      String year=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_POS_HIS_YEAR);
      String month=(String)request.getParameter(DMInterfaceKey.CONTROL_SELECT_POS_HIS_MONTH);
      
      if( DataMigrationDao.checkPOS_HISFile(year,month))
      {
    	  
  %>

  <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="Import" method="post">
          <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>" value="<%=DMInterfaceKey.ACTION_POS_PAY_HISTORY_IMPORT %>">
  </form>
    <script>
   ...
   //   boolean checkFromLCSPhysicalAmount = false;
    //  if(vecCurrentStockProducts.size()==0)
     // {
        Vector vec= DataMigrationDao.getTmpPOS_PAY_HISData();
      ...
      if(vec.size()!=0){
    	 ...
    	 fileid= DataMigrationDao.insertNewPOS_PAY_HISFile(year,month,"active",userID);  
    	  
      }  
        
        for(int i = 0;i<vec.size();i++)
        {
          POS_PAY_HISModel model = (POS_PAY_HISModel) vec.get(i);
          String phaseDate =model.getPHASE_DATE();
          String memName =model.getMEMO_NAME();
          String code =model.getCODE();
          String toatalAmount =model.getTOTAL_AMOUNT();
          ...
          String saleTax =model.getSALES_TAX();
          
          String regDate =model.getREG_DATE();
          
          String comment1 =model.getCOMMENT1();
          
          String comment2 =model.getCOMMENT2();
          
          String memoDesc =model.getMEMEO_DESC();
          
          String deductionFees =model.getDEDUCTION_FEES();
          
          String remainedDeduction =model.getREMAINED_DEDUCTION();
          
          String fiananceDate =model.getFINANCE_DATE();
          
          String budgetId =model.getBUDGET_ID();
          
          String budgetname =model.getBUDGET_NAME();
  ...
  //  Long fileid = DataMigrationDao.insertNewPaymentCashFile(year,month,"active");
     ...
     DataMigrationDao.insertNewPOS_PAY_HISData(fileid,phaseDate,memName,code,toatalAmount,amount,done,msg,saleTax ,regDate,comment1,comment2,memoDesc,deductionFees,remainedDeduction,fiananceDate,budgetId,budgetname); 
    ...
    //else
     ...
     //   contractError.setErrorMsg("Stock Products have values for today");
      ...
      // }
     DataMigrationDao.deleteTmpPOS_PAY_HISData();
