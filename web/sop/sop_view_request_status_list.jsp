...
<%                
  ...
  System.out.println("The vector size isssssssssssssssss " + vecRequestList.size());
...
%>
      ...
      <tr>
        ...
        <%
        out.print("<INPUT class=button type=button value=\" Search \" name=\"searchrequest\" id=\"searchrequest\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_SEARCH_REQUEST_STATUS_LIST+"';"+
                  "SOPform.submit();\">");
    ...
    for(int i=0;i<vecRequestList.size();i++)
    {
      ...
      <tr class=<%=InterfaceKey.STYLE[i%2]%>>
        ...
        <td>
        <%
          if(requestStatusId.compareTo(SOPInterfaceKey.CONST_REQUEST_STATUS_DELETED)==0 || requestStatusId.compareTo(SOPInterfaceKey.CONST_REQUEST_STATUS_PAID)==0)
          {
        %>
          ...
          <select name='<%=SOPInterfaceKey.INPUT_SELECT_REQUEST_STATUS%>_<%=requestId%>' id='<%=SOPInterfaceKey.INPUT_SELECT_REQUEST_STATUS%>_<%=requestId%>'>
            ...
            for(int j=0;j<vecRequestStatusList.size();j++)
            {
              ...
              if(requestStatusIdX.compareTo(SOPInterfaceKey.CONST_REQUEST_STATUS_DELETED)==0 || requestStatusIdX.compareTo(SOPInterfaceKey.CONST_REQUEST_STATUS_PAID)==0 )
              {
              %>
...
<%
    ...
    if(vecRequestList.size()!=0)
    {    
      out.print("<INPUT class=button type='button' value=\" Update Request Status \" name=\"updaterequest\" id=\"updaterequest\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_UPDATE_REQUEST_STATUS_LIST+"';"+
                 "checkBeforeSubmit();\">");
    }
%>
    </center>
...
if(objDataHashMap.containsKey(InterfaceKey.HASHMAP_KEY_ACTION))
  {
    ...
    if(strAction.compareTo(SOPInterfaceKey.ACTION_SEARCH_REQUEST_STATUS_LIST)==0)
    {
