...
<script>
 ...
 function setSearchValues(creationDateFrom,creationDateTo,paymentDateFrom,paymentDateTo,warehouseId)
  ...
  HashMap productReport = (HashMap)objDataHashMap.get(InterfaceKey.HASHMAP_KEY_ADDITIONAL_COLLECTION);
...
<%--      <tr class=TableTextNote>
      ...
      <tr>
        ...
        <%
        out.print("<INPUT class=button type=button value=\" Search \" name=\"searchScratch\" id=\"searchScratch\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_SEARCH_PRODUCT_REPORT+"';"+
                  "SOPform.submit();\">");
...
<%
 Set set = productReport.entrySet();
...
%>
  ...
  <%
   ...
   Map mapdcms = Collections.synchronizedMap(new TreeMap(productReport));
...
<br><br>
    ...
    <input type=button class=button value="Export to Excel" 
        ...
        SOPInterfaceKey.ACTION_EXPORT_PRODUCT_REPORT%>'; document.SOPform.submit();">
        </center>
...
if(objDataHashMap.containsKey(InterfaceKey.HASHMAP_KEY_ACTION))
  {
    ...
    if(strAction.compareTo(SOPInterfaceKey.ACTION_SEARCH_PRODUCT_REPORT)==0)
    {
