...
<html>
 ...
 $(document).ready(function(){$("#LCSState").tablesorter(); });
</script>
<script>
        ...
        function setSearchValues(franchiseCode,itemCode,lcsItemCode,startDate,endDate,lcsItemDescription)
        {
          ...
          document.getElementById("<%=SOPInterfaceKey.INPUT_TEXT_LCS_ITEM_CODE%>").value = lcsItemCode;
          ...
          document.getElementById("<%=SOPInterfaceKey.INPUT_TEXT_LCS_ITEM_DESCRIPTION%>").value = lcsItemDescription;
        }
...
%>
  ...
  <input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_ACTION%>"
      ...
      <TR class=TableTextNote>
        ...
        <td><input type='text' name='<%=SOPInterfaceKey.INPUT_TEXT_FRANCHISE_CODE%>' value="Any" ></td>
        <td>Item Code</td>
        <td><input type='text' name='<%=SOPInterfaceKey.INPUT_TEXT_ITEM_CODE%>' value="Any"></td>
        <td>LCS Item Code</td>
        <td><input type='text' name='<%=SOPInterfaceKey.INPUT_TEXT_LCS_ITEM_CODE%>' value="Any"></td>
      </TR>
      <TR class=TableTextNote>
        <td>LCS Date From</td>
        <td><Script>drawCalender('<%=SOPInterfaceKey.INPUT_TEXT_START_DATE%>','Any');</script></td>
        <td>To</td>
        <td><script>drawCalender('<%=SOPInterfaceKey.INPUT_TEXT_END_DATE%>','Any');</script></td>
        <td>LCS Item Desc</td>
        <td><input type='text' name='<%=SOPInterfaceKey.INPUT_TEXT_LCS_ITEM_DESCRIPTION%>' value="Any"></td>
      </TR>
      <tr>
        <td align='center' colspan=6>
        <%
        out.println("<INPUT class=button type=submit value=\" Search \" name=\"searchrequest\" id=\"searchrequest\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_SEARCH_LCS_STATE+"';"+
                  "\">");
    ...
    %>
    <br><br>
<TABLE class="tablesorter" id="LCSState">
	  <thead>    
    <tr>
      <th >Franchise Code</th>
      <th>Item Code</th>
      <th>LCS Item Code</th>
      <th>LCS Item Description</th>
      <th>LCS Date</th> 
      <th>Quantity</th>  
      </tr>
      </thead>
    ...
    <%
      for(int i=0; i<result.size();i++)
      {
        ...
        String sourceItemCode = model.getSourceItemCode();
        if(sourceItemCode == null)sourceItemCode ="";
        out.println("<td>"+sourceItemCode+"</td>");
        String sourceItemDesc = model.getSourceItemDesc();
        if(sourceItemDesc == null)sourceItemDesc = "";
        out.println("<td>"+sourceItemDesc+"</td>");
    ...
    <tr>
      <td colspan=8 align=center>
        ...
        SOPInterfaceKey.ACTION_EXCEL_LCS_STATE%>'; document.SOPform.submit();">
      </td>
    </tr>
    </table>
    <br><br>
...
Utility.logger.debug("The total rows issssssssssss " + totalRows);
 ...
 if(rowNum>=1)
  {
    //out.println("<a href=\"#\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_PREVIOUS_LCS_STATE+"';"+"document.SOPform.submit();\">Previous</a></td>");

    out.println("<td align = 'right'><INPUT id=button4 class=button type=button value=\"<<\" name=button3 onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_PREVIOUS_LCS_STATE+"';"+"document.SOPform.submit();\">");
  }
  else
  {
  ...
  {
    //out.println("<td align = 'right'><a href=\"#\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_NEXT_LCS_STATE+"';"+"document.SOPform.submit();\">next</a>");

    out.println("<INPUT id=button3 class=button type=button value=\">>\" name=button3 onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_NEXT_LCS_STATE+"';"+"document.SOPform.submit();\"></td>");
  }
  else
  {
...
<%
  if(searchModel != null)
      ...
      "','"+searchModel.getItemCode()+"','"+searchModel.getSourceItemCode()+"','"+startDate+"','"+endDate+"','"+searchModel.getSourceItemDesc()+"');</script>");
%>
</body>
</html>
