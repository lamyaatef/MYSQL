...
<html>
  ...
  <body>
    ...
    <script type="text/javascript">
      ...
      document.UserManagment.<%out.print(InterfaceKey.HASHMAP_KEY_ACTION);%>.value="<%out.print(SOPInterfaceKey.ACTION_UPDATE_USER_CHANNEL_LIST);%>";
    ...
    <form action="<%out.print(appName);%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="UserManagment" method="post">
      ...
      value="<%out.print(SOPInterfaceKey.ACTION_UPDATE_USER_CHANNEL_LIST);%>">

      <input type="hidden" name="<%out.print(SOPInterfaceKey.INPUT_HIDDEN_CHANNEL_ID);%>" 
      value="">
...
//        out.println("<input type=\"hidden\" name=\""+ReportInterfaceKey.INPUT_HIDDEN_REPORT_ID+"\">");
        ...
        Vector vecChannelList = (Vector) dataHashMap.get (InterfaceKey.HASHMAP_KEY_ADDITIONAL_COLLECTION) ;
        Vector vecUserChannelList = (Vector) dataHashMap.get (InterfaceKey.HASHMAP_KEY_ADDITIONAL_COLLECTION_2) ;
        for(int k = 0; k<vecUserChannelList.size(); k++)
        {
              //PersonReportModel personReportModel = (PersonReportModel)vecPersonReportList.get(k);
              UserChannelModel userChannelModel = (UserChannelModel)vecUserChannelList.get(k);
              String personId = userChannelModel.getUserId();
              String channelId = userChannelModel.getChannelId();
          ...
          int channelListSize = vecChannelList.size();
        ...
        for(int j = 0; j<vecChannelList.size(); j++)
        {
          ...
          chanelModel channelModel = (chanelModel)vecChannelList.get(j);
          String SchannelId = channelModel.getCHANNEL_ID();
          int channelId = Integer.parseInt(SchannelId);
	   ...
	   int nChannelId = Integer.parseInt(userChannelModel.getChannelId());
	   //System.out.println("nlableId isssssssssssss " + nlableId);
         //int nGroupId =  Integer.parseInt(personReportModel.getGroupId());
         int nPersonId = Integer.parseInt(userChannelModel.getUserId());
