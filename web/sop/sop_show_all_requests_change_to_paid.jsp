...
<script>
        ...
        function setSearchValues(requestId,dcmName,requestCreateDateFrom,requestCreateDateTo,requestStatus)
        {
          <!--document.getElementById("<%=SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_CODE%>").value = requestCode;-->
          ...
          document.getElementById("<%=SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_CREATION_DATE_FROM%>").value = requestCreateDateFrom;
...
<%                
  ...
  if(objDataHashMap.containsKey(SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_ID)){searchRequestId = (String)objDataHashMap.get(SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_ID);}
  if(objDataHashMap.containsKey(SOPInterfaceKey.INPUT_SEARCH_SELECT_DCM_NAME)){searchDcmName = (String)objDataHashMap.get(SOPInterfaceKey.INPUT_SEARCH_SELECT_DCM_NAME);}
  if(objDataHashMap.containsKey(SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_CREATION_DATE_FROM)){searchRequestCreateDateFrom = (String)objDataHashMap.get(SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_CREATION_DATE_FROM);}
  if(objDataHashMap.containsKey(SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_CREATION_DATE_TO)){searchRequestCreateDateTo = (String)objDataHashMap.get(SOPInterfaceKey.INPUT_SEARCH_TEXT_REQUEST_CREATION_DATE_TO);}
  if(objDataHashMap.containsKey(SOPInterfaceKey.INPUT_SEARCH_SELECT_REQUEST_STATUS)){searchRequestStatus = (String)objDataHashMap.get(SOPInterfaceKey.INPUT_SEARCH_SELECT_REQUEST_STATUS);}  
  if(objDataHashMap.containsKey(SOPInterfaceKey.INPUT_HIDDEN_WAREHOUSE_ID)){searchWarehouseId = (String)objDataHashMap.get(SOPInterfaceKey.INPUT_HIDDEN_WAREHOUSE_ID);}
...
%>
      ...
      <tr>
        ...
        <%
        out.print("<INPUT class=button type=button value=\" Search \" name=\"searchrequest\" id=\"searchrequest\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_SEARCH_REQUEST_CHANGE_TO_PAID+"';"+
                  "SOPform.submit();\">");
        ...
        %>
          ...
          <select name='<%=SOPInterfaceKey.INPUT_SELECT_REQUEST_STATUS%>_<%=requestId%>' id='<%=SOPInterfaceKey.INPUT_SELECT_REQUEST_STATUS%>_<%=requestId%>'>
            ...
            for(int j=0;j<vecRequestStatusList.size();j++)
            {
              ...
              if(requestStatusIdX.compareTo(SOPInterfaceKey.CONST_REQUEST_STATUS_ACTIVE)==0 || requestStatusIdX.compareTo(SOPInterfaceKey.CONST_REQUEST_STATUS_PAID)==0 )
              {
              %>
...
<%
    if(vecRequestList.size()!=0)
    {    
      out.print("<INPUT class=button type='button' value=\" Update Request Status \" name=\"updaterequest\" id=\"updaterequest\" onclick=\"document.SOPform."+InterfaceKey.HASHMAP_KEY_ACTION+".value = '"+SOPInterfaceKey.ACTION_UPDATE_REQUEST_STATUS_CHANGE_TO_PAID+"';"+
                 "checkBeforeSubmit();\">");
    }
%>
    </center>
...
if(objDataHashMap.containsKey(InterfaceKey.HASHMAP_KEY_ACTION))
  {
    ...
    if(strAction.compareTo(SOPInterfaceKey.ACTION_SEARCH_REQUEST_CHANGE_TO_PAID)==0)
    {
