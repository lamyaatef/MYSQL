...
String pageHeader="Payment Excel Management";
...
Vector PaymentTypeExcelModels = (Vector)dataHashMap.get(MemoInterfaceKey.VECTOR_SQL_TEMPLATES);                  
%>
...
function toggleChange()
{
	...
	document.getElementById("1").style.display='none';
	document.getElementById("2").style.display='none';
	document.<%=formName%>.<%= MemoInterfaceKey.HIDDEN_SQL_TEMPLATE_ID %>.value="2";
}
    ...
    <body>
        ...
        <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="<%=formName%>" method="post">
            ...
            <td colspan="2" align="center"> Edit </td>
             ...
             <option value=""></option>
             <%
                if(model.getSqlTempId() == -1)
                {
                   %>
                   <option value="specific" selected >Specific</option>
                   <option value="temp">Payment Excel Template</option>
                   <% 
                }
                else
                {
                   ...
                   <option value="temp"  selected >Payment Excel Template</option>
                   <% 
                }
             %>
             ...
             <td><textarea rows="20" cols="20" name="<%=PaymentInterfaceKey.CONTROL_NEW_EXCEL_SHEET_SQL_STATEMENT %>" id="<%=PaymentInterfaceKey.CONTROL_NEW_EXCEL_SHEET_SQL_STATEMENT %>" ><%=model.getExcelSheetSqlStatement() %></textarea>
             </td>
             </tr>
             ...
             <td>
             <select name="<%= PaymentInterfaceKey.PAYMENT_NAME_TEMPLATE %>" id="<%= PaymentInterfaceKey.PAYMENT_NAME_TEMPLATE %>" >
             ...
             for(int i = 0 ; i < PaymentTypeExcelModels.size() ; i++)
             {
                ...
                SqlTemplateModel modelName=(SqlTemplateModel)PaymentTypeExcelModels.get(i);
                if(model.getExcelSheetSqlStatement().equals(modelName.getTemplateName()))
                {
                   %>
                    <option value="<%=  modelName.getTemplateName() %>" selected><%= modelName.getTemplateName() %></option>
                   <%
                }
                else
                {
                   %>
                    <option value="<%= modelName.getTemplateName() %>"><%= modelName.getTemplateName() %></option>
                   <%
                }   
