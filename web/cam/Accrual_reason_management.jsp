...
<%
        ...
        String page_header="Accrual Reason List";
//get data hashmap specidifed b4 in kpihandler
        
        HashMap dataHashMap = (HashMap) request.getAttribute(InterfaceKey.HASHMAP_KEY_DTO_OBJECT);
        Vector all_deduction_reason = (Vector) dataHashMap.get(AccrualInterfaceKey.VECTOR_ALL_REASONS);
...
function submitNew()
{
document.<%=form_name%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=AccrualInterfaceKey.ACTION_NEW_REASON%>";
document.<%=form_name%>.submit();
} 
function submitEdit(rowId)
{
document.<%=form_name%>.<%=AccrualInterfaceKey.CONTROL_HIDDEN_REASON_ID%>.value=rowId;
document.<%=form_name%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=AccrualInterfaceKey.ACTION_EDIT_REASON%>";
document.<%=form_name%>.submit();
}
function submitUpdateStatuses()
{
document.<%=form_name%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=AccrualInterfaceKey.ACTION_UPDATE_REASON_STATUS%>";
document.<%=form_name%>.submit();
}

function submitSearch()
{
  document.<%=form_name%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=AccrualInterfaceKey.ACTION_VIEW_REASON%>";
  document.<%=form_name%>.submit();
}
</script>
<html>
    ...
    <body>
        ...
        <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="<%=form_name%>" method="post">
            ...
            <input type="hidden" name="<%=AccrualInterfaceKey.CONTROL_HIDDEN_REASON_ID%>" value="-1">   

             <TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=2 cellPadding=1 width="100%" border=1>
              <TR class=TableHeader>
              <TD>Name</TD>
              <TD>
                  <input type="text" name="<%=AccrualInterfaceKey.CONTROL_TEXT_SEARCH_REASON_NAME%>">
              </TD>
              <TD>Status</TD>
              <TD>
                  <%
                   ...
                   {%>
                   <select name="<%=AccrualInterfaceKey.CONTROL_SELECT_SEARCH_REASON_STATUS%>" id="<%=AccrualInterfaceKey.CONTROL_SELECT_SEARCH_REASON_STATUS%>">
                   <option>
                   <%
            ...
            %>
             ...
             if(all_deduction_reason_status!=null && all_deduction_reason_status.size()>0)
             {%>
             <select name="<%=AccrualInterfaceKey.CONTROL_SELECT_REASON_STATUS+item.getReason_id()%>" id="<%=AccrualInterfaceKey.CONTROL_SELECT_REASON_STATUS+item.getReason_id()%>">
             <%
