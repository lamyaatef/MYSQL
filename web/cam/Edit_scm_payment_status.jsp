...
<%
        ...
        String form_name = "payment_edit_form";
        String page_header="Edit SCM Payment Status";
        HashMap dataHashMap = (HashMap) request.getAttribute(InterfaceKey.HASHMAP_KEY_DTO_OBJECT);
        String userID = (String)dataHashMap.get(InterfaceKey.HASHMAP_KEY_USER_ID);
        Vector all_scm = (Vector) dataHashMap.get(PaymentInterfaceKey.VECTOR_ALL_SCM_PAYMENT);
        PaymentScmStatusModel item = (PaymentScmStatusModel)all_scm.get(0);
...
document.<%=form_name%>.<%=PaymentInterfaceKey.CONTROL_HIDDEN_SCM_PAYMENT_ID%>.value = scm_id;
document.<%=form_name%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=PaymentInterfaceKey.ACTION_DO_EDIT_SCM_PAYMENT_STATUS%>";
