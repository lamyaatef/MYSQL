SELECT 
    ...
    xt4.arpu_commission AS "ARPU Commission",
    xt1.total_item_amount as "Total Lines",
    (xt1.xCommission_Total - abs(nvl(xt4.arpu_commission, 0))) as  "Total Amount",
    xt2.xCommission_Deduction  as "SD Deduction", 
    (xt1.xCommission_Total - abs(nvl(xt4.arpu_commission, 0)) - abs(nvl(xt2.xCommission_Deduction,0))) as  "Net Amount"    
FROM
    ...
    LEFT JOIN
    (SELECT  tt6.DCM_ID, SUM(tt6.COMMISSION_ITEM_AMOUNT * tt7.COMMISSION_FACTOR_VALUE) AS arpu_commission
        FROM 
            ...
            JOIN COMMISSION_TYPE_CATEGORY tt9 ON (tt5.COMMISSION_TYPE_CATEGORY_ID = tt9.COMMISSION_TYPE_CATEGORY_ID)
           JOIN COMMISSION_TYPE tt10 ON (tt9.COMMISSION_TYPE_ID = tt10.COMMISSION_TYPE_ID)            
        WHERE tt9.COMMISSION_TYPE_CATEGORY_NAME='ARPU Commission' AND tt1.MEMO_ID = ?           
        GROUP BY
