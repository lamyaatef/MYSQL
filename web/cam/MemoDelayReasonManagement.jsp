...
String pageHeader="Memo Delay Reason Management";
...
function submitViewEdit(reasonId)
{
	...
	document.<%=formName%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=MemoInterfaceKey.ACTION_VIEW_UPDATE_MEMO_DELAY_REASON%>";
...
{
	document.<%=formName%>.<%=MemoInterfaceKey.HIDDEN_MEMO_REASON_ID%>.value = reasonId;
	document.<%=formName%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=MemoInterfaceKey.ACTION_UPDATE_MEMO_DELAY_REASON%>";
	document.<%=formName%>.submit();

}
function submitDelete(reasonId){
	document.<%=formName%>.<%=MemoInterfaceKey.HIDDEN_MEMO_REASON_ID%>.value = reasonId;
	document.<%=formName%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=MemoInterfaceKey.ACTION_DELETE_MEMO_DELAY_REASON%>";
	document.<%=formName%>.submit();
}
function submitViewAdd()
{
	document.<%=formName%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=MemoInterfaceKey.ACTION_VIEW_ADD_MEMO_DELAY_REASON%>";
	document.<%=formName%>.submit();
}
function submitAdd()
{
	document.<%=formName%>.<%=InterfaceKey.HASHMAP_KEY_ACTION%>.value="<%=MemoInterfaceKey.ACTION_ADD_MEMO_DELAY_REASON%>";
	document.<%=formName%>.submit();
}
</script>
<html>
    ...
    <body>
        ...
        <form action="<%=appName%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="<%=formName%>" method="post">
            ...
            <%if(action.equals(MemoInterfaceKey.ACTION_VIEW_MEMO_DELAY_REASONS)||action.equals(MemoInterfaceKey.ACTION_UPDATE_MEMO_DELAY_REASON)||action.equals(MemoInterfaceKey.ACTION_DELETE_MEMO_DELAY_REASON)||action.equals(MemoInterfaceKey.ACTION_ADD_MEMO_DELAY_REASON)) {%>
            <%if(reasons!=null||reasons.size()!=0){%>
           <TABLE style="BORDER-COLLAPSE: collapse" cellSpacing="2" cellPadding="1" width="100%" border="1">
            <TR class="TableHeader">
            <td colspan="4" align="center">Memo Delay Reasons</td>
            </TR> 
            <tr class="TableHeader">
            <td>reason</td>
            <td>Description</td>
            ...
            else if(action.equals(MemoInterfaceKey.ACTION_VIEW_ADD_MEMO_DELAY_REASON)){%>
            ...
            <%}else if(action.equals(MemoInterfaceKey.ACTION_VIEW_UPDATE_MEMO_DELAY_REASON)){
