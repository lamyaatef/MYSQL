...
<%
	...
	String tableId = (String) request
        ...
        String POSStatus=(String) request.getParameter(SCMInterfaceKey.POS_STATUS_CHANGE_LIST);

        String paymentStatus=(String) request.getParameter(SCMInterfaceKey.PAYMENT_STATUS_CHANGE_LIST);
	...
	Vector report = importReport.getReport();
        ...
        int RefusedPOSsNotExist=0;
        int RefusedPOSsNothaspayment=0;
        int RefusedPOSsNotActive=0;

	int numOfRecordsUpdated = importReport.getNumOfRecordsUpdated();

        refusedPOSs=POSDAO.beforeChangePOSFromExcel();

        for(int i=0; i<refusedPOSs.size() ;i++ )
            {
                if(refusedPOSs.get(i).getCaseExist()==1)
                    {
                        RefusedPOSsNotExist++;
                    }
                if(refusedPOSs.get(i).getCaseExist()==2)
                    {
                        RefusedPOSsNothaspayment++;
                    }
                if(refusedPOSs.get(i).getCaseExist()==3)
                    {
                        RefusedPOSsNotActive++;
                    }
            }

         if(!POSStatus.equals("0"))
                    {
                      changedPOSs= POSDAO.changeposstatusfromExcel(POSStatus,strUserID,reason);
                    }
         if(!paymentStatus.equals("0"))
                    {
                      changedPOSs = POSDAO.changepaymentstatusfromExcel(paymentStatus,strUserID,reason);
                    }

    {
		...
		if (report.size() == 0 & RefusedPOSsNotExist ==0 && !POSStatus.equals("0"))
		...
		} else if (report.size() == 0 & RefusedPOSsNotExist ==0 && RefusedPOSsNothaspayment ==0 && RefusedPOSsNotActive ==0 &&!paymentStatus.equals("0"))

		{
			...
			printToStream("</TABLE>", out);
				...
				printToStream("<h3>",out);
                                if (!POSStatus.equals("0")){
				printToStream(RefusedPOSsNotExist+" POSs were refused",out);
                                }
                                if(!paymentStatus.equals("0"))
                                {
                                 printToStream(RefusedPOSsNotExist+RefusedPOSsNothaspayment+" POSs were refused",out);
                                }
				printToStream("</h3>",out);
				...
				String distLink=PoiWriteExcelFile.ExportExcelPOSChanges(refusedPOSs,"POS", baseDirectory);
				...
				printToStream(RefusedPOSsNotExist+" POSs were refused",out);
                                ...
                                if(!paymentStatus.equals("0"))
                                {
                                 printToStream(RefusedPOSsNotExist+RefusedPOSsNothaspayment+RefusedPOSsNotActive+" POSs were refused",out);
                                }
				printToStream("</h3>",out);
                                }
                                
				String distLink=PoiWriteExcelFile.ExportExcelPOSChanges(refusedPOSs,"Payment", baseDirectory);
				dataHashMap.put(InterfaceKey.EXPORT_FILE_PATH,distLink);
                                dataHashMap.put(InterfaceKey.MODULE_SUB_PATH,"scm"+Slach+"upload"+Slach);
                                session.setAttribute(InterfaceKey.HASHMAP_KEY_DTO_OBJECT ,dataHashMap);
                                %>
