...
<%
	...
	STKDAO converter = new STKDAO();
	Vector <CaseModel> resultcase=new Vector<CaseModel>();

	resultcase=converter.beforeInsertSTK_POS();
	
	int importedRows =converter.InsertSTK_POS(strUserID); 
	int RefusedRows =resultcase.size();
	
	{
...

		{
			...
			printToStream("Operation Completed Successfully "
							+importedRows+" STKs and POSs were imported", out);
			printToStream("</h3>", out);
		...
		{	
			...
			printToStream("</TABLE>", out);
				...
				{
					CaseModel current = (CaseModel) resultcase.get(i);
				...
				if(current.getstkCase()!=0)
				{
					faildSTK++;
                                ...
                                printToStream("<h3>",out);
				printToStream(importedRows+" Records were imported",out);
				printToStream("</h3>",out);
				printToStream("<h3>",out);
				printToStream(report.size()+" Records were refused",out);
				printToStream("</h3>",out);
				printToStream("<h3>",out);
				printToStream(faildPOS+" POSs were refused",out);
				printToStream("</h3>",out);
				printToStream("<h3>",out);
				printToStream(faildSTK+" STKs were refused",out);
				printToStream("</h3>",out);
				String distLink=PoiWriteExcelFile.ExportExcel(resultcase,baseDirectory);
