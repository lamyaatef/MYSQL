...
if(jobId != null)
{
  ...
  if(workerHashMap == null)
  {
      ...
      out.println("<input type=\"hidden\" name=\""+InterfaceKey.HASHMAP_KEY_ACTION+"\""+
                    " value=\""+ContractRegistrationInterfaceKey.ACTION_IMPORT_EXCEL_FROM_ARCHIVING_PROCESS_REJECT+"\">");
...
String baseDirectory =request.getRealPath("/cr/uploadArchiving/");
...
printToStream("<script type=\"text/javascript\">",out,pw);
  ...
  Vector<String> simNumVec = ContractDao.getSimNum(con);

   for(int i =0 ; i<simNumVec.size();i++)

   {
       String simNumber = simNumVec.get(i);

       ContractDao.updateSimRejectedStatus(simNumber);
       //String contractId = ContractDao.getContractIdBySimNum(con , simNumVec.get(i));
       //if(contractId.compareTo("0") == 0)
         //  {
             //ErrorInfo sheetError = new ErrorInfo();
             //sheetError.setCellName("");
             //sheetError.setErrorMsg("Sim Num is Invalid ... ");
             //sheetError.setRowNum(i+1);
             //report.add(sheetError);
           //}
       //else
         //  {
            //ContractDao.insertContractStatusRecord(contractId, strUserID, "6");
           //}

   }
  ContractDao.deleteTmpSim(con);


 /* ImportArchivingWorkerReject iaWorker = new ImportArchivingWorkerReject(tableId,operation,strUserID,batchId,reportImport);
  String strJobId = WorkerDataManagement.addWorker(iaWorker);
  HashMap workerHashMap = WorkerDataManagement.getWorkerData(strJobId);
  if(workerHashMap == null)
  {
      out.println("<form action=\""+appName+"/servlet/com.mobinil.sds.web.controller.WebControllerServlet\" name=\"ImportArchivingForm\" method=\"post\">");
  ...
  else
  {
    ...
    WorkerDataManagement.removeWorkerData(strJobId);
  ...
  else
  {
      ...
      if(batchExistsInVerificationAndOenedForArchiving)
      {
        ...
        HashMap contractWarningTypes = ContractDao.getContractWarningTypeId(con);
          ...
          for(int j=0;j<contractModels.size();j++)
          {
            ...
            if(contractStatusName.compareTo(ContractRegistrationInterfaceKey.CONST_CONTRACT_STATUS_ACCEPTED_VERIFY)==0)
            {
              ...
              if(contractsFromArchiving.containsKey(contractMainSIMNumbre))
              {
                ...
                if(contractWarningTypes.containsKey(warningTypeName))
                {
                  user_warnings[0] = (String)contractWarningTypes.get(warningTypeName);
                }
                else
                {
                  user_warnings[0] = "";  
                ...
                if(user_warnings[0].compareTo("")!=0)
                {
                  ...
                  ContractDao.updateContract(contractId, strUserID, sheetId, batchId, "5" , user_warnings , contractSystemWarningModelVector);
                }
              }
              else
              {
...
if (report.size()>0 || reportImport.size()>0)
{
  ...
  printToStream("</TABLE>",out,pw);
    ...
    {
      printToStream("<br><br><h3>All Data Updated ...</h3>", out, pw);
    }
