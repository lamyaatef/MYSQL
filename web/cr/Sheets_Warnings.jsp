  ...
  <body>
    <Center>
      <H2>Sheet Warnings</H2>
        <form action="<%out.print(appName);%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet" name="SheetWarnings" method="post">
        ...
        <%
          showSheetWarnings(request, response, out);
        %>
      </form>
    </Center>
  </body>
 ...
 * 
*/
  public void showSheetWarnings(HttpServletRequest request, HttpServletResponse response, JspWriter out)
  throws ServletException, IOException
  {
    ...
    if(dataHashMap != null)
    {
      ...
      {
        HashMap sheetPhaseWarnings = (HashMap)dataHashMap.get(InterfaceKey.HASHMAP_KEY_COLLECTION);
        Vector warningStatuses = (Vector)dataHashMap.get(InterfaceKey.HASHMAP_KEY_ADDITIONAL_COLLECTION);
        if(! sheetPhaseWarnings.isEmpty())
        {
          ...
          for(int i=0; i<sheetPhaseWarnings.size(); i++)
          {
            String strPhaseName = (String)sheetPhaseWarnings.keySet().toArray()[i];
            ...
            Vector warnings = (Vector)sheetPhaseWarnings.get(strPhaseName);
            if(warnings.isEmpty())
            {
              out.println("<TR");
            ...
            {
              out.println("<TR class=TableHeader>");
              out.println("<td align=middle size=\"35%\">Warning</td>");
              out.println("<td align=middle size=\"35%\">Warning Description</td>");
              out.println("<td align=middle size=\"20%\">Warning Status</td>");
              out.println("<td align=middle size=\"10%\">Edit</td></tr>");
              for(int j=0; j<warnings.size(); j++)
              {
                SheetWarningModel newSheetWarningModel = (SheetWarningModel)warnings.elementAt(j);
                String warningID = newSheetWarningModel.getSheetWarningID();
                String warning = newSheetWarningModel.getSheetWarningName();
                String warningDesc = newSheetWarningModel.getSheetwarningDesc();
                if(warningDesc == null)
                {
                  warningDesc = "";
                }
                String sheetWarningStatusID = newSheetWarningModel.getSheetwarningStatusID();
                out.println("<TR>");
                ...
                out.println("<SELECT name="+ContractRegistrationInterfaceKey.CONTROL_SELSCT_NAME_PREFIX+warningID+
                            " onchange=\"document.SheetWarnings.update.disabled = false;\">");
                for(int k=0; k<warningStatuses.size(); k++)
                {
                  ...
                  if(sheetWarningStatusID.compareTo(warningStatusID)==0)
                  {
                    out.print("selected");
                  }
                ...
                out.println("onclick=\"document.SheetWarnings."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+ContractRegistrationInterfaceKey.ACTION_EDIT_SHEET_WARNING+"';");
                out.println("document.SheetWarnings."+ContractRegistrationInterfaceKey.CONTROL_HIDDEN_WARNING_ID+".value='"+warningID+"';");
                out.println("SheetWarnings.submit();\">");
                out.println("</TD>");
                out.println("</TR>");
              }
            }
        ...
        out.print("onclick=\"document.SheetWarnings."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+ContractRegistrationInterfaceKey.ACTION_NEW_SHEET_WARNING+"';SheetWarnings.submit();\">");
        out.println("<input class=button type=\"button\" name=\"update\" value=\"Update Warnings\" ");
        out.print("onclick=\"document.SheetWarnings."+InterfaceKey.HASHMAP_KEY_ACTION+".value='"+ContractRegistrationInterfaceKey.ACTION_UPDATE_SHEET_WARNINGS_STATUSES+"';SheetWarnings.submit();\" disabled>");
        out.println("</td>"); 
        out.println("</tr>"); 
        out.println("</table>"); 
      }
