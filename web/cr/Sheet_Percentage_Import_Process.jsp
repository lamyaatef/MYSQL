...
<%@page import="com.mobinil.sds.core.system.cr.batch.model.BatchModel"%><%@ 
...
String baseDirectory =request.getRealPath("/cr/uploadSheetPercentage/");
...
for(int i = 0;i<vecSheetPercebtage.size();i++)
{
  ...
  BatchModel batchModel = BatchDao.getBatch(tmBatchId);
  
  if(batchModel != null)
  {
      if(inttmSheetLocalAuthPercentage >= 0 && inttmSheetLocalAuthPercentage <= 100)
      {
          BatchDao.updateBatchLocalAuthPercentage(tmBatchId , inttmSheetLocalAuthPercentage);
      }
      else
      {
          contractError =  new ErrorInfo();
          contractError.setCellName("Batch Local Auth Percentage");
  ...
  {
     contractError = new ErrorInfo();
      contractError.setCellName("Batch Id");
      contractError.setErrorMsg("Batch Id doesn't exist in the system");
      contractError.setRowNum(inttmRowNum);
      report.add(contractError);
  }
}
