...
String baseDirectory =request.getRealPath("/sfr/upload/");
...
printToStream("</script>",out,pw);  
  ...
  Vector vecSFRExcelFile = SheetDAO.getTmpSfrExcelFile(strUserID,con);
  ...
  for(int i = 0;i<vecSFRExcelFile.size();i++)
  {
    boolean insertSheetFlag = true;
    Long lSheetId = null;
    SheetImportModel sheetImportModel = (SheetImportModel)vecSFRExcelFile.get(i);
    ...
    Vector existingSheets = SheetDAO.getSheetsByFilter("",sheetSerial,"1",con);
      ...
      else
      {
        if(existingSheets.size() == 0)
        {
          ...
          if(insertBatchFlag)
          {
            ...
            lBatchId = SheetDAO.insertBatch((String)lastValidValue.get(SheetImportModel.AGENT_ID),"4",con);
            vecBatchIds.add(lBatchId);
            if(lBatchId != null)
            {
              SheetDAO.insertBatchStatus(lBatchId,"4",strUserID,con);
            }
          }
        ...
        else
        {
          ...
          sheetError.setErrorMsg("Sheet Serial exists in the system with accepted status.");
          sheetError.setRowNum(Integer.parseInt(tmRowNum));
          report.add(sheetError); 
        }
