...
<HTML>
   <HEAD>
      ...
      <TITLE>Data View Sorting Criteria </TITLE>
   </HEAD>
   ...
   <!-- <Center>
      <H2>Data View Sorting Criteria</H2>
    </Center>-->
    <FORM id=formDataViewOrderBy name=formDataViewOrderBy action="" method=post>
      <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
         <tr class=TableHeader>
           ...
           <tr class=TableHeader>
            <td class=TableHeader><font color=darkblue size=3>Data View Order By</td>
           </tr>
          <tr class=TableHeader>
            <td class=TableHeader>&nbsp;</td>
           </tr>
        ...
        <TR>
          <TD class=TableHeader vAlign=top colSpan=9>Add sorting criteria
            <A onclick="field_RowSet_add('2','half_day');">
            <IMG alt="Click Here to add new sorting criteria ... " 
    ...
    function move_up(argobj)
    {
          ...
          selectlist = eval("document.formDataViewOrderBy.field__R"+i+"__C2");
          ...
          selectlistabove = eval("document.formDataViewOrderBy.field__R"+j+"__C2");
    ...
    var arrSortingTypes           =         new Array(
//    ('',''), 
    new Array('Ascending','<%=QueryBuilderInterfaceKey.ORDER_TYPE_ASCENDING%>'),
    new Array('Descending','<%=QueryBuilderInterfaceKey.ORDER_TYPE_DESCENDING%>'));
...
//    new Array('Constant','<%=QueryBuilderInterfaceKey.FIELD_TYPE_CONST_FIELD%>'),
    ...
    new Array('',''));
      ...
      function popUpFieldsWindow(argID)
      {
            ...
            eval("nFieldType = document.formDataViewOrderBy."+strFieldTypeControlIndex+".value");
            if(nFieldType != '')
            {
                eval("nFieldID = document.formDataViewOrderBy."+strFieldIDControlIndex+".value");
                ...
                if(objWin != null)
                {
                   ...
                   eval("document.formDataViewOrderBy.field__R"+nRowIndex+"__C2.value = '"+objWin[0]+"'");
                } 
           }
           else
      ...
      function popUpFields(argObjID)
      {
        ...
        // Delete all fields   
            var nCount = Number(eval("document.formDataViewOrderBy."+strPopUpColumnID+".length"));
            for(var i=0;i<nCount;i++)
                eval("document.formDataViewOrderBy."+strPopUpColumnID+".remove(0);");

      eval("nSelectedValue = document.formDataViewOrderBy."+strID+".value");
           ...
           try
           {
              var arrPairs=new Array();
                  ...
                  for(var j =0;j<Number(arrPairs.length);j=Number(j+2))    
                  {
                       ...
                       strFieldName = "";
                       eval("document.formDataViewOrderBy."+strPopUpColumnID+".add(objOption);");
              }  
...
//        default: alert("No Matching");
      ...
      {
        document.formDataViewOrderBy.field_count.value = field.RowSet.getRowCount();
        ix = document.formDataViewOrderBy.field_count.value = field.RowSet.getRowCount();
        field.RowSet.add();
        ...
        for (i = 0; i < arrSortingTypes.length; i++)
        {
          if (arrSortingTypes[i] == current_value)
          {
            field.RowSet.getCell(ix+1,2).cellElement.selectedIndex = i;
          }
        }
      }
                    ...
                    function field_removeRows()
                    {
                      ...
                      if (i==true){
                        for(var i=field.getRowCount();i>=1;i--){
                          if(field.getCell(i,5).getValue()==true){
                        ...
                        for(var i=field.getRowCount();i>=1;i--){
                          if(field.getCell(i,5).getValue()==true){
                            field.getCell(i,5).setValue() == false
                          }//end if
                        }//end for
                      }//end else
                    }
                    ...
                    field.ColumnHeaders.add(null,null,"Field Name",null,345,"center",DG_SELECT,fieldArray)
                    field.ColumnHeaders.add(null,null,"(+)",null,20,"center",DG_BUTTON," (+) ",null,"onClick = popUpFieldsWindow(this.id)");                    
                    field.ColumnHeaders.add(null,null,"Sorting Type",null,190,"center",DG_SELECT,arrSortingTypes)
...
//                    field_RowSet_add('','');
    ...
    {
            var nFieldsCount = 1;
            if(top.objNewDataView.m_arrDataViewOrderBy.length > 0)
            {
               for(var i=0;i< Number(top.objNewDataView.m_arrDataViewOrderBy.length); i++)
                {
                    if(top.objNewDataView.m_arrDataViewOrderBy[i] != null)
                    {
                        eval("field.RowSet.add("+Number(nFieldsCount)+");");
                        eval("document.formDataViewOrderBy.field__R"+Number(nFieldsCount)+"__C1.value = '"+ top.objNewDataView.m_arrDataViewOrderBy[i].m_nOrderByElementTypeID+"';");
                        eval("popUpFields('field__R"+Number(nFieldsCount)+"__C1');");
                        eval("document.formDataViewOrderBy.field__R"+Number(nFieldsCount)+"__C2.value = '"+ top.objNewDataView.m_arrDataViewOrderBy[i].m_nOrderByIndex+"';");
                        eval("document.formDataViewOrderBy.field__R"+Number(nFieldsCount)+"__C4.value = '"+ top.objNewDataView.m_arrDataViewOrderBy[i].m_strOrderByType+"';");
                        nFieldsCount++;
                     }
                }// End For
...
function addToCart()
{
  top.objNewDataView.m_arrDataViewOrderBy.length = 0;
  ...
  for(var i=1;i<=field.getRowCount();i++)
  {
    if(field.getCell(i,1).getValue() == "<%=QueryBuilderInterfaceKey.FIELD_TYPE_DATAVIEW_FIELD%>")
    {
          if(field.getCell(i,2).getValue() != '' && FieldsIDs.length > 0)
          {
              ...
              var objOrderBy = new DataViewOrderBy(0,'<%=QueryBuilderInterfaceKey.FIELD_TYPE_DATAVIEW_FIELD%>',field.getCell(i,4).getValue());
              objOrderBy.m_objOrderByField = objSelectedField;
              objOrderBy.m_nOrderByIndex = nIndex;
              top.objNewDataView.addOrderBy(objOrderBy);
          }
      }
/*    else
      {
...
//            objSelectedField.addFeatures(null,null,field.getCell(i,5).getValue(),field.getCell(i,3).getValue(),field.getCell(i,2).getValue());
          ...
          else
          {
              ...
              {
                var nIndex = field.getCell(i,2).getValue();
                var objOrderBy = new DataViewOrderBy(0,'<%=QueryBuilderInterfaceKey.FIELD_TYPE_FUNCTION_FIELD%>',field.getCell(i,4).getValue());
                objOrderBy.m_nOrderByIndex = nIndex;
                top.objNewDataView.addOrderBy(objOrderBy);
              }
//          }
