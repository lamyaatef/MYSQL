...
<html>
  ...
  <script>  
      ...
      document.formChannel.<%out.print(InterfaceKey.HASHMAP_KEY_ACTION);%>.value='<%out.print(SIPInterfaceKey.ACTION_SIP_EDIT_CHANNEL);%>';
      document.formChannel.<%out.print(SIPInterfaceKey.CONTROL_HIDDEN_CHANNEL_ID);%>.value=channelId;        
      formChannel.submit();
      }

      function deleteChannel(channelId) {
          document.formChannel.<%out.print(InterfaceKey.HASHMAP_KEY_ACTION);%>.value='<%out.print(SIPInterfaceKey.ACTION_SIP_DELETE_CHANNEL);%>';
          document.formChannel.<%out.print(SIPInterfaceKey.CONTROL_HIDDEN_CHANNEL_ID);%>.value=channelId;        
          formChannel.submit();
          }    
      function newChannel() {
      document.formChannel.<%out.print(InterfaceKey.HASHMAP_KEY_ACTION);%>.value='<%out.print(SIPInterfaceKey.ACTION_SIP_NEW_CHANNEL);%>';
      formChannel.submit();
      }       
    </script>
    <CENTER>
    ...
    <%
     ...
     out.println("<input type=\"hidden\" name=\""+InterfaceKey.HASHMAP_KEY_USER_ID+"\""+
                  " value=\""+"\">");
     out.println("<input type=\"hidden\" name=\""+SIPInterfaceKey.CONTROL_HIDDEN_CHANNEL_ID+"\""+
             " value=\""+"\">");                                 
...
try{
  ...
  Vector vecChannelList = (Vector) objDataHashMap.get(SIPInterfaceKey.CHANNEL_VECTOR) ;
...
out.println("<th ><font size=2>Channel Name</font></a></th><th><font size=2>Edit</font></a></th><th><font size=2>Delete</font></a></th></thead><tbody>");

  for (int i=0; i<taskListSize; i++)
  {
  
    SIPChannelModel channelModel=  (SIPChannelModel) vecChannelList.elementAt (i);    
    ...
    out.println("<td width=\"10%\" align=center><input type=button value=\"Delete\" onclick=\"deleteChannel("+channelId+");\"></td>");
