...
function loadValues(control,value)
  {
    document.formChannelDetails.control.value=value;
        ...
        {
          if(NonBlank(document.formChannelDetails.<%out.print(SIPInterfaceKey.CONTROL_TEXT_CHANNEL_NAME);%>, true, 'Channel Name'))
          {  
          document.formChannelDetails.<%out.print(InterfaceKey.HASHMAP_KEY_ACTION);%>.value='<%out.print(SIPInterfaceKey.ACTION_SIP_SAVE_CHANNELS);%>'
...
Vector channelInfo = (Vector) objDataHashMap.get(SIPInterfaceKey.CHANNEL_MODEL) ;
...
out.println("<TD class=TableTextNote colSpan=4><INPUT style=\"WIDTH: 452px; HEIGHT: 22px\" size=66"
              + " value=\"" +"\""  
              + " name=\""+SIPInterfaceKey.CONTROL_TEXT_CHANNEL_NAME+"\">");
...
if (channelInfo!=null)
{
	SIPChannelModel cm = (SIPChannelModel)channelInfo.elementAt(0);
        ...
        out.println("<script>");
        out.println("document.formChannelDetails."+SIPInterfaceKey.CONTROL_TEXT_CHANNEL_NAME+".value='"+channelName+"';");        
        out.println("document.formChannelDetails."+InterfaceKey.HASHMAP_KEY_USER_ID+".value='"+userId+"';");        
        out.println("document.formChannelDetails."+SIPInterfaceKey.CONTROL_HIDDEN_CHANNEL_ID+".value='"+channelId+"';");        
        out.println("</script>");
}       
