			...
			
			{
				...
				{		
					...
					var temppp ='<%out.print(appName);%>/servlet/com.mobinil.sds.web.controller.WebControllerServlet?';
				 	...
				 	temppp+='quartar='+document.getElementById('month').value+'&';				 					 					 	
					temppp+='sip_report_name='+document.getElementById('sip_report_name').value+'';
					
				
								
				document.getElementById('POSForm').action=temppp;						
...
<body onkeypress = "if(event.keyCode==13){POSForm.submit();}">    
<form name="POSForm" id="POSForm"  method="post" enctype="multipart/form-data">
<input type="hidden" name="<%=InterfaceKey.HASHMAP_KEY_USER_ID %>" id="<%=InterfaceKey.HASHMAP_KEY_USER_ID %>" value="<%=userId %>" />
<center>
      <h2>Upload List Of ExPos and Non-ExPos Per Superdealer</h2>
      </center>
<input type="hidden" name="sip_action" id="sip_action" value="<%=SIPInterfaceKey.ACTION_HIDDEN_PARAM_NAME_UPLOAD_DATA %>">
<input type="hidden" name="<%= SIPInterfaceKey.ACTION_HIDDEN_PARAM_NAME_PREVIOUS_ACTION%>" id="<%= SIPInterfaceKey.ACTION_HIDDEN_PARAM_NAME_PREVIOUS_ACTION %>" value="<%=SIPInterfaceKey.ACTION_UPLOAD_EXPOS_PER_SUPERDEALER %>">
<input type="hidden" name="<%= SIPInterfaceKey.HIDDEN_PARAM_NAME_SIP_REPORT %>" id="<%= SIPInterfaceKey.HIDDEN_PARAM_NAME_SIP_REPORT%>" value="<%= SIPInterfaceKey.HIDDEN_PARAM_SIP_REPORT_SD%>">
    <center>
    ...
    <tr>      
              <td colspan=2 align="center">    <input class="button" type="button" name="Submit" value="Submit your file" onclick="javascript:load_image('distUploadFile','POSForm','POSForm');">
